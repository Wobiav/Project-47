...
function spawnOgres(){
  if(frameCount%110 === 0 ){
    ogre = createSprite(-50, Math.round(random(height-100, height-700)), 20, 20)
    ogre.addImage(ogreImage);
    ogre.scale = 0.15;
    ogre.velocityX= Math.round(random(5, 15));

    ogre.lifetime = width/ogre.velocityX+50;
...
function spawnDragons(){
  if(frameCount%120 === 0 ){
    dragon = createSprite(-100, Math.round(random(height-600, height-1000)), 20, 20)
    dragon.addImage(dragonImage);
    dragon.scale = 0.4;
    dragon.velocityX= Math.round(random(7, 15));

    dragon.lifetime = width/dragon.velocityX+100;
...
function spawnGoblins(){
  if(frameCount%75 === 0){
    goblin = createSprite(-50 , Math.round(random(height-100, height-500, 20, 20)))
    goblin.addImage(goblinImage);
    goblin.scale = 0.15;
    goblin.velocityX = Math.round(random(3, 10));

    goblin.lifetime = width/goblin.velocityX+50;
